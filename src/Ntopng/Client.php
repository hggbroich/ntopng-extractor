...
readonly class Client {
    ...
    private function getJsonFromResponse(ResponseInterface $response): string {
        return mb_convert_encoding($response->getContent(), 'UTF-8', 'UTF-8');
    }

    public function getInterfaces(): InterfacesResponse {
        $json = $this->ntopngClient->request('GET', '/lua/rest/v2/get/ntopng/interfaces.lua');
        return $this->serializer->deserialize($this->getJsonFromResponse($json), InterfacesResponse::class, 'json');
    }
    ...
    public function getFlows(int $interfaceId, int $page = 1, int $perPage = 100): ActiveFlowsResponse {
        ...
        return $this->serializer->deserialize($this->getJsonFromResponse($json), ActiveFlowsResponse::class, 'json');
    }
    ...
    public function getActiveFlowsList(int $interfaceId, int $start = 0, int $length = 10000): ActiveFlowListResponse {
        ...
        return $this->serializer->deserialize($this->getJsonFromResponse($json), ActiveFlowListResponse::class, 'json');
    }
    ...
    public function getLocalHosts(int $interfaceId, int $page = 1, int $perPage = 100): ActiveHostsResponse {
        ...
        return $this->serializer->deserialize($this->getJsonFromResponse($json), ActiveHostsResponse::class, 'json');
    }
}
